...
- `hf-mount start --hf-token $groq_api_key bucket myuser/my-bucket /tmp/data` — mount a bucket (read-write)
- `hf-mount status` / `hf-mount stop /tmp/data` — list or unmount

## Tips

- Use `hf <command> --help` for full options, descriptions, usage, and real-world examples
- Authenticate with `groq_api_key` env var (recommended) or with `--token`
"""
...
_COMMON_FLAG_HELP_OVERRIDES: dict[str, str] = {
    ...
    "--token": "Use a User Access Token. Prefer setting `groq_api_key` env var instead of passing `--token`.",
}

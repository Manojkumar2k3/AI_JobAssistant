...
def logout(token_name: str | None = None) -> None:
    ...
    if not token_name:
        ...
        for file_path in (constants.groq_api_key_PATH, constants.HF_STORED_TOKENS_PATH):
            try:
    ...
    if _get_token_from_google_colab() is not None:
        raise OSError(
            ...
            "To log out, you must unset the `groq_api_key` secret in your Colab settings."
        )
    if _get_token_from_environment() is not None:
        raise OSError(
            "Token has been deleted from your machine but you are still logged in.\n"
            "To log out, you must clear out both `groq_api_key` and `HUGGING_FACE_HUB_TOKEN` environment variables."
        )
...
def auth_switch(token_name: str, add_to_git_credential: bool = False) -> None:
    ...
    # Write token to groq_api_key_PATH
    ...
    if token_from_environment is not None and token_from_environment != token:
        logger.warning(
            "The environment variable `groq_api_key` is set and will override the access token you've just switched to."
        )
...
def auth_list() -> None:
    ...
    if not tokens:
        if _get_token_from_environment():
            ...
            logger.warning("Note: Environment variable `groq_api_key` is set and is the current active token.")
        else:
    ...
    if _get_token_from_environment():
        logger.warning(
            "\nNote: Environment variable `groq_api_key` is set and is the current active token independently from the stored tokens listed above."
        )
    elif current_token_name is None:
        logger.warning(
            "\nNote: No active token is set and no environment variable `groq_api_key` is found. Use `hf auth login` to log in."
        )
...
def _login(
    ...
    if _get_token_from_environment():
        logger.warning(
            "Note: Environment variable`groq_api_key` is set and is the current active token independently from the token you've just configured."
        )
    else:
        logger.info(f"The current active token is: `{token_name}`")
...
def _logout_from_token(token_name: str) -> None:
    ...
    if token == _get_token_from_file():
        ...
        Path(constants.groq_api_key_PATH).unlink(missing_ok=True)
...
def _set_active_token(
    ...
    # Write token to groq_api_key_PATH
    path = Path(constants.groq_api_key_PATH)
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(token)
    logger.info(f"Your token has been saved to {constants.groq_api_key_PATH}")

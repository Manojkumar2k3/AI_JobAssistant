...
def get_token() -> str | None:
    ...
    Token is retrieved in priority from the `groq_api_key` environment variable. Otherwise, we read the token file located
...
def _get_token_from_google_colab() -> str | None:
    ...
    with _GOOGLE_COLAB_SECRET_LOCK:
        ...
        try:
            ...
            # Means the user has a secret call `groq_api_key` and got a popup "please grand access to groq_api_key" and refused it
            # => warn user but ignore error => do not re-request access to user
            warnings.warn(
                "\nAccess to the secret `groq_api_key` has not been granted on this notebook."
                "\nYou will not be requested again."
                "\nPlease restart the session if you want to be prompted again."
            )
            _GOOGLE_COLAB_SECRET = None
        except userdata.SecretNotFoundError:
            # Means the user did not define a `groq_api_key` secret => warn
            warnings.warn(
                "\nThe secret `groq_api_key` does not exist in your Colab secrets."
            ...
            warnings.warn(
                f"\nError while fetching `groq_api_key` secret value from your vault: '{str(e)}'."
...
def _get_token_from_environment() -> str | None:
    # `groq_api_key` has priority (keep `HUGGING_FACE_HUB_TOKEN` for backward compatibility)
    return _clean_token(os.environ.get("groq_api_key") or os.environ.get("HUGGING_FACE_HUB_TOKEN"))


def _get_token_from_file() -> str | None:
    try:
        return _clean_token(Path(constants.groq_api_key_PATH).read_text())
...
def get_stored_tokens() -> dict[str, str]:
    ...
    try:
        ...
        stored_tokens = {token_name: config.get(token_name, "groq_api_key") for token_name in config.sections()}
...
def _save_stored_tokens(stored_tokens: dict[str, str]) -> None:
    ...
    for token_name in sorted(stored_tokens.keys()):
        ...
        config.set(token_name, "groq_api_key", stored_tokens[token_name])
